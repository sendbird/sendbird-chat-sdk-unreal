...
public class Sendbird : ModuleRules
{
    public Sendbird(ReadOnlyTargetRules Target) : base(Target)
    {
        ...
        {
            string iosPath = Path.Combine(ModuleDirectory, "lib", "ios", "libSendbirdChat.a");
        ...
        {
            string win32Path = Path.Combine(ModuleDirectory, "lib", "windows", "x86", "SendbirdChat.lib");
            if (File.Exists(win32Path))
            {
                isLibrarySupported = true;
                PublicAdditionalLibraries.Add(win32Path);
            }
        }
        else if (Target.Platform == UnrealTargetPlatform.Mac)
        {
            string macPath = Path.Combine(ModuleDirectory, "lib", "mac", "libSendbirdChat.a");
